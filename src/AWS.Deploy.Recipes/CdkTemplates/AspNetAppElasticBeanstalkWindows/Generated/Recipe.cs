...
using AspNetAppElasticBeanstalkWindows.Configurations;
using Constructs;
...
namespace AspNetAppElasticBeanstalkWindows
{
    ...
    public class Recipe : Construct
    {
            ...
            : base(scope, "Recipe")
        {
            ...
            // Write the Beanstalk manifest file to the .NET Zip bundle.
            SetupAWSDeploymentManifest(settings, props.DotnetPublishZipPath);
        ...
        public void SetupAWSDeploymentManifest(Configuration settings, string dotnetZipFilePath)
        {
            const string MANIFEST_FILENAME = "aws-windows-deployment-manifest.json";

            var iisWebSite = !string.IsNullOrEmpty(settings.IISWebSite) ? settings.IISWebSite : "Default Web Site";
            var iisAppPath = !string.IsNullOrEmpty(settings.IISAppPath) ? settings.IISAppPath : "/";
            ...
            using (var jsonWriter = new Utf8JsonWriter(jsonStream))
            {
                ...
                jsonWriter.WriteStringValue(iisWebSite);
                jsonWriter.WritePropertyName("iisPath");
                jsonWriter.WriteStringValue(iisAppPath);
