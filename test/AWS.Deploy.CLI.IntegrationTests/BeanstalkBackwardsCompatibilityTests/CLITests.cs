...
namespace AWS.Deploy.CLI.IntegrationTests.BeanstalkBackwardsCompatibilityTests
{
    ...
    public class CLITests(TestContextFixture fixture)
    {
        [Fact]
        ...
        {
            var projectPath = fixture.TestAppManager.GetProjectPath(Path.Combine("testapps", "WebAppNoDockerFile", "WebAppNoDockerFile.csproj"));
            var deployArgs = new[] { "deploy", "--project-path", projectPath, "--application-name", fixture.EnvironmentName, "--diagnostics", "--silent", "--region", "us-west-2", "--apply", "Existing-ElasticBeanStalkConfigFile-Linux-SelfContained.json" };
            InMemoryInteractiveService interactiveService = null!;
            Assert.Equal(CommandReturnCodes.SUCCESS, await fixture.ServiceCollection.RunDeployToolAsync(deployArgs,
                provider =>
                {
