...
namespace AWS.Deploy.CLI.IntegrationTests.SaveCdkDeploymentProject
{
    public class RedeploymentTests : IDisposable
    {
        ...
        public async Task AttemptWorkFlow()
        {
            ...
            await Utilities.CreateCDKDeploymentProjectWithRecipeName(projectPath, "Custom ECS Fargate Recipe", "1", compatibleDeploymentProjectPath);

            // Create an incompatible CDK deployment project using the App runner recipe
            await Utilities.CreateCDKDeploymentProjectWithRecipeName(projectPath, "Custom App Runner Recipe", "2", incompatibleDeploymentProjectPath);
